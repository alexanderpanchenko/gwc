...
\begin{frame}
  ...
  \begin{itemize}
		...
		\item semantic classes \cite{panchenko:2018:SemanticClasses} 
		\end{itemize}
...
\begin{frame}
\frametitle{Related work: \\Knowledge-Based Sense Embeddings}
\begin{itemize}
	\item AutoExtend~\cite{rothe-schutze:2015:ACL-IJCNLP}
\end{itemize}
\begin{center}
...
\begin{frame}{Related work: \\ Knowledge-Free Sense Embeddings}

\begin{itemize}
\item Adagram~\cite{bartunov2016breaking}
...
\item $p(\beta_{wk}|\alpha)$ -- a priory probability of the $k$-th sense of the word $w$;
\item $p(z_i|x_i,\mathbf{\beta})$ -- probability of observing word $x_i$ in the sense $z_i$;
\item $\prod_{j=1}^C p(y_{ij}|z_i,x_i,\theta)$ -- probability of a context $C$.
\end{itemize}
